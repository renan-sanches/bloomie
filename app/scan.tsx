...
export default function ScanScreen() {
  ...
  const handleAddPlant = async () => {
    ...
    try {
      ...
      // Add to store (and firestore) - returns plant with ID
      const createdPlant = await addPlant(newPlant);
      ...
      if (createdPlant?.id) {
        router.push(`/plant/${createdPlant.id}` as any);
      } else {
        // Fallback to dashboard if ID not returned
        router.push("/");
      }
